...
from search_l3s_gateway import db, bcrypt

from search_l3s_gateway.util.datetime_util import (
    utc_now,
    get_local_utcoffset,
    make_tzaware,
    localized_dt_string,
...
from search_l3s_gateway.util.result import Result
...
class User(db.Model):
    ...
    def password(self, password):
        log_round = current_app.config.get("BCRYPT_LOG_ROUNDS")
        hash_bytes = bcrypt.generate_password_hash(password, log_round)
    ...
    def find_by_public_id(cls, public_id):
        return db.session.execute(db.select(cls).filter_by(public_id=public_id)).first()
    ...
    def encode_access_token(self):
        ...
        if current_app.config["TESTING"]:
            ...
            expire_in = now + timedelta(hours=token_age_h, minutes=token_age_m)

        payload = dict(exp=expire_in, iat=now, sub=self.public_id, admin=self.admin)
